...
- Review checkpoint: can you justify why read-only MCP was the safest first addition, and why `GITHUB_TOKEN` scope plus Docker/runtime boundaries now matter to MCP risk classes?

### Day 3
- Time budget: 45 minutes
- Read: [labs/lab-tools-and-environment.md](labs/lab-tools-and-environment.md)
- Do: run the lab's suggested local validation commands if your environment is ready, and inspect [/.mcp.json](../../.mcp.json) to classify `filesystem-readonly`, `postgres-readonly`, `github`, `playwright`, `docker`, and `fetch-url`
...
- justify a read-only-first MCP/tool strategy and explain why token scope and runtime impact change MCP risk without changing server names
